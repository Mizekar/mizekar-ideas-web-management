...
class Dashboard extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      assessmentResultsPeriod: "Last30Days",
      profilesStatisticsPeriod: "Last30Days",
      assessmentResultsLoading: true,
      profilesStatisticsLoading: true,
    }
  ...
  async assessmentResults() {
    ...
    this.setState({
      assessmentResults: chart,
      assessmentResultsOption: chartOpts,
      assessmentResultsLoading: false
    })
  }

  async handleChangeAssessmentResultsPeriod(period) {
    await this.setState({
      assessmentResultsPeriod: period
    })
  ...
  async profilesStatistics() {
    let response = await get("ideas/statistics/profiles/" + this.state.profilesStatisticsPeriod, {
      timePeriod: this.state.profilesStatisticsPeriod
    })
    ...
    this.setState({
      profilesStatistics: chart,
      profilesStatisticsOption: chartOpts,
      profilesStatisticsLoading: false
    })
  }
  async handleChangeProfileStatisticsPeriod(period) {
    await this.setState({
      profilesStatisticsPeriod: period
    })
        ...
        <Row>
          <Col>
            <Card className="p-3">
              <Row>
                <Col sm="5">
                  <CardTitle className="mb-0">ارزیابی سوژه ها</CardTitle>
                  <div className="small text-muted"></div>
                </Col>
                <Col sm="7" className="d-none d-sm-inline-block">
                  <ButtonToolbar className="float-right" aria-label="Toolbar with button groups">
                    <ButtonGroup className="ml-3" aria-label="First group">
                      <Button color="outline-secondary"
                              onClick={() => this.handleChangeAssessmentResultsPeriod("Last7Days")}
                              active={this.state.assessmentResultsPeriod === "Last7Days"}>7 روز
                        گذشته</Button>
                      <Button color="outline-secondary"
                              onClick={() => this.handleChangeAssessmentResultsPeriod("Last30Days")}
                              active={this.state.assessmentResultsPeriod === "Last30Days"}>30 روز
                        گذشته</Button>
                      <Button color="outline-secondary"
                              onClick={() => this.handleChangeAssessmentResultsPeriod("Total")}
                              active={this.state.assessmentResultsPeriod === "Total"}>تمام زمان
                        ها</Button>
                    </ButtonGroup>
                  </ButtonToolbar>
                </Col>
              </Row>
              {!this.state.assessmentResultsLoading &&
              <div className="chart-wrapper" style={{height: 300 + 'px', marginTop: 40 + 'px'}}>
                <Line
                  data={this.state.assessmentResults}
                  options={this.state.assessmentResultsOption}
                  height={300}
                />
              </div>
              }
        ...
        <Row>
          <Col>
            <Card className="p-3">
              <Row>
                <Col sm="5">
                  <CardTitle className="mb-0">کاربران جدید</CardTitle>
                  <div className="small text-muted"></div>
                </Col>
                <Col sm="7" className="d-none d-sm-inline-block">
                  <ButtonToolbar className="float-right" aria-label="Toolbar with button groups">
                    <ButtonGroup className="ml-3" aria-label="First group">
                      <Button color="outline-secondary"
                              onClick={() => this.handleChangeProfileStatisticsPeriod("Last7Days")}
                              active={this.state.profilesStatisticsPeriod === "Last7Days"}>7 روز
                        گذشته</Button>
                      <Button color="outline-secondary"
                              onClick={() => this.handleChangeProfileStatisticsPeriod("Last30Days")}
                              active={this.state.profilesStatisticsPeriod === "Last30Days"}>30 روز
                        گذشته</Button>
                      <Button color="outline-secondary"
                              onClick={() => this.handleChangeProfileStatisticsPeriod("Total")}
                              active={this.state.profilesStatisticsPeriod === "Total"}>تمام زمان
                        ها</Button>
                    </ButtonGroup>
                  </ButtonToolbar>
                </Col>
              </Row>
              {!this.state.profilesStatisticsLoading &&
              <div className="chart-wrapper" style={{height: 300 + 'px', marginTop: 40 + 'px'}}>
                <Line
                  data={this.state.profilesStatistics}
                  options={this.state.profilesStatisticsOption}
                  height={300}
                />
              </div>
              }
